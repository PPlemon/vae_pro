...
def main():
    ...
    print(charset)
    model = MoleculeVAE()
    if os.path.isfile('data/vae_model_44(1).h5'):
        model.load(charset, 'data/vae_model_44(1).h5', latent_rep_size=LATENT_DIM)
    else:
        model.create(charset, latent_rep_size=LATENT_DIM)

    check_pointer = ModelCheckpoint(filepath='data/vae_model_44(1).h5', verbose=1, save_best_only=True)
    ...
    tbCallBack = TensorBoard(log_dir="TensorBoard/vae_model_44(1)")
