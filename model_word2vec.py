...
def most_similar(word2id,w): #找相似性Topk个词
    ...
    model = load_model('word2vec_w3_35.h5') #载入模型 在数据集较大的时候用空间换时间
...
if __name__ == '__main__': 
    ...
    word_size = 35 #词向量维度
    window = 3 #窗口大小
    nb_negative = 1 #随机负采样的样本数
    min_count = 0 #频数少于min_count的词将会被抛弃
    nb_worker = 4 #读取数据的并发数
    nb_epoch = 10 #迭代次数，由于使用了adam，迭代次数1～2次效果就相当不错
    ...
    #print(w2v_vector['A'])
    output_smiles = open('data/w2v_vector_35_new_w3.pkl', 'wb')
    pickle.dump(w2v_vector, output_smiles)
    output_smiles.close()

    output_id2word = open('data/id2word_w3_35.pkl', 'wb')
    output_word2id = open('data/word2id_w3_35.pkl', 'wb')
